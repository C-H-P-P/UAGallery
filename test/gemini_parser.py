...
ПОТОЧНА ДАТА: {current_year}-{current_month:02d}-01
{limit_instruction}

Для кожної виставки визнач:
- "title": назва виставки (рядок)
- "start_date": дата у форматі YYYY-MM-DD.
  Правила дат:
  * Якщо вказано тільки день і місяць (наприклад "5 травня") — підстав поточний рік {current_year}
  * Якщо місяць у майбутньому відносно {current_year}-{current_month:02d} — рік {current_year}
  * Якщо місяць вже минув цього року — можливо {current_year} або {current_year - 1}, обирай логічно
  * Якщо дата зовсім відсутня — null
- "end_date": дата завершення YYYY-MM-DD. Ті самі правила. Якщо немає — null
- "image_url": АБСОЛЮТНИЙ URL зображення виставки.
  Шукай у тексті рядки вигляду:
  * ![будь-який текст](URL) — це Markdown-картинка, бери URL
  * [IMAGE:URL] — спеціальний маркер, бери URL
  * Якщо URL відносний (починається з /images/ або /img/) — додай базовий домен з base_url
  * Якщо картинок немає — null
- "description": короткий опис, 2-3 речення
- "artists": масив імен художників (може бути порожнім [])
